...
export default function ManagerCalendar() {
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      setIsLoading(true);
      try {
        const response = await fetch("http://localhost:3535/taskalldetail");
        if (!response.ok) {
        ...
        setTaskData(allTasks.filter((task: Task) => task.name === localStorage.getItem("nameofuser")));// Set all data when owner11 logs in
